...
class TemporalConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1):
        ...
        self.conv2 = nn.Conv2d(
            ...
            in_channels,
            groups=in_channels,
            kernel_size=(kernel_size_2, 1),   # Conv along the temporal dimension only
            padding=(pad_2, 0),
            dilation=(dilation_2,1),
            stride=(stride, 1)
        )
        self.conv3 = nn.Conv2d(
            ...
            in_channels,
            groups=in_channels,
            kernel_size=(kernel_size_3, 1),   # Conv along the temporal dimension only
            padding=(pad_3, 0),
            dilation=(dilation_3,1),
            stride=(stride, 1)
        )
    ...
    def forward(self, x):
        N, Cv_in, T, V_node = x.shape # (N, Cv_in, T, V_node)
        x = self.conv4(self.conv1(x) + self.conv2(x) + self.conv3(x))
        x = self.bn(x)
        return x
    
class VertexTemporalConv(nn.Module):
    def __init__(self, node_in_channels, node_out_channels, kernel_size=9, stride=1):
        super(VertexTemporalConv,self).__init__()
...
class MGNBlock(nn.Module):
    def __init__(self, node_in_channels, node_out_channels, adj_mat):
        ...
        # Updating functions
        self.Hv_agg = nn.Conv2d(3*node_in_channels, node_out_channels, kernel_size = (1,1))
        self.relu = nn.ReLU()
    ...
    def forward(self, fv):
        ...
        fv_source_node = torch.einsum('ncv,vy->ncy', fv, self.adj_mat.transpose(0,1)).contiguous().view(N, T, Cv_in, V_node).permute(0,2,1,3) # (N,Cv_in,T,V_nodes)
        fv_target_node = torch.einsum('ncv,vy->ncy', fv, self.adj_mat).contiguous().view(N, T, Cv_in, V_node).permute(0,2,1,3) # (N,Cv_in,T,V_nodes)
        fv = fv.contiguous().view(N,T, Cv_in, V_node).permute(0,2,1,3) # (N,Cv_in,T,V_nodes)
        fvp = torch.cat((fv,fv_source_node, fv_target_node), dim = 1) # (N,3Cv_in,T,V_node)
...
class Model(nn.Module):
    def __init__(self):
        ...
        num_class, vertices, num_person, motion_channels, joint_channels, bone_channels = 10, 20, 1, 3, 3, 3
    ...
    def forward(self, joint_data, joint_motion_data, bone_data):
        
        # joint_data has shape (N, Cv, T, V_node, M) where Cv = 3 & V_node = 20
        # joint_motion_data has shape (N, Cv, T, V_node, M) where Cv = 3 & V_node = 20
        # bone_data has shape (N, Cv, T, V_node, M) where Cv = 3 & V_node = 20
