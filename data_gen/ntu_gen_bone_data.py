import os
import numpy as np
from numpy.lib.format import open_memmap
from tqdm import tqdm

pairs = {
    'ntu/xview': ( # Direction of node ordering is away from node 21.
        (1, 2), (2, 21), (3, 21), (4, 3), (5, 21),
        (6, 5), (7, 6), (8, 7), (9, 21), (10, 9),
        (11, 10), (12, 11), (13, 1), (14, 13), (15, 14),
        (16, 15), (17, 1), (18, 17), (19, 18), (20, 19),
        (21, 21), (22, 23), (23, 8), (24, 25), (25, 12)
    ),
    'ntu/xsub': (
        (1, 2), (2, 21), (3, 21), (4, 3), (5, 21),
        (6, 5), (7, 6), (8, 7), (9, 21), (10, 9),
        (11, 10), (12, 11), (13, 1), (14, 13), (15, 14),
        (16, 15), (17, 1), (18, 17), (19, 18), (20, 19),
        (21, 21), (22, 23), (23, 8), (24, 25), (25, 12)
    ),
    'ntu_120/xsub': (
        (1, 2), (2, 21), (3, 21), (4, 3), (5, 21),
        (6, 5), (7, 6), (8, 7), (9, 21), (10, 9),
        (11, 10), (12, 11), (13, 1), (14, 13), (15, 14),
        (16, 15), (17, 1), (18, 17), (19, 18), (20, 19),
        (21, 21), (22, 23), (23, 8), (24, 25), (25, 12)
    ),
    'ntu_120/xsetup': (
        (1, 2), (2, 21), (3, 21), (4, 3), (5, 21),
        (6, 5), (7, 6), (8, 7), (9, 21), (10, 9),
        (11, 10), (12, 11), (13, 1), (14, 13), (15, 14),
        (16, 15), (17, 1), (18, 17), (19, 18), (20, 19),
        (21, 21), (22, 23), (23, 8), (24, 25), (25, 12)
    ),
    'kinetics': (
        (0, 0), (1, 0), (2, 1), (3, 2), (4, 3), (5, 1),
        (6, 5), (7, 6), (8, 2), (9, 8), (10, 9), (11, 5),
        (12, 11), (13, 12), (14, 0), (15, 0), (16, 14), (17, 15)
    )
}

sets = {'train', 'val'}
datasets = {'ntu/xview','ntu/xsub','ntu_120/xsub','ntu_120/xsetup','kinetics'}

def gen_bone_data():
    """Generate bone data from joint data for NTU skeleton dataset"""
    for dataset in datasets:
        for set in sets:
            print(dataset, set)
            data = np.load('../data/{}/{}_data_joint.npy'.format(dataset, set))
            N, C, T, V, M = data.shape
            bone = open_memmap(
                '../data/{}/{}_data_bone.npy'.format(dataset, set),
                dtype='float32',
                mode='w+',
                shape=(N, 3, T, 18, M))
           

            # Copy the joints data to bone placeholder tensor
            for edge, (v1, v2) in tqdm(enumerate(pairs[dataset])):
                # Reduce class index for NTU datasets
                if dataset != 'kinetics':
                    v1 -= 1
                    v2 -= 1
                # Assign bones to be joint1 - joint2, the pairs are pre-determined and hardcoded
                bone[:, :, :, edge, :] = data[:, :, :, v1, :] - data[:, :, :, v2, :]

                
if __name__ == '__main__':
    gen_bone_data()