...
def pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4]):
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        for i_p, person in enumerate(skeleton):
            if person.sum() == 0:
                continue
            # For all `person`, compute the `mask` which is the non-zero channel dimension After subtraction, the original frames which had no movement may have a value. In order to ensure that these frames still show zero motion, this mask is necessary.
